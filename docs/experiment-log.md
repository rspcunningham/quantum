...
| 29 | `284c00a` | Compile-time single-qubit gate fusion. Consecutive 1q gates on the same qubit fused into a single gate by multiplying their 2x2 matrices at compile time (numpy). Handles dense, diagonal, and permutation 1q gates uniformly; near-identity results dropped entirely. Applied at ≥17q where per-gate MPS cost justifies compile overhead. `parametric_brickwork_rt_24` 1.71x, `brickwork_entangler_rt_24` 1.31x, `hw_efficient_ansatz_rt_24` 1.20x faster. 130 improved, 83 regressed. Geomean 1.01x. | 66.5s (151 cases) | Worked (1.08x) |
