...
Overall geometric mean across all 780 cells: **60.3x faster** than Aer (156 common cases, 5 shot counts, 0.1ms floor). At high shot counts (10K), native is **79.2x faster** (780/780 cells native wins). At low shot counts (1), native wins 50.1x. CDF caching skips state evolution entirely on repeated calls; sparse CDF caching reduces searchsorted from O(log 2^n) to O(log K) for circuits with few outcomes; probability-weighted branching computes exact dynamic-circuit distributions analytically; compilation caching amortizes fusion passes; inverse-pair cancellation collapses all 50 roundtrip circuits to identity in O(n) time.
